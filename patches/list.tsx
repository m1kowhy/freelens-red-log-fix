...
/**
 * Removes ANSI color sequences from Kubernetes/container logs.
 * This prevents pod errors/warnings from being rendered as red text.
 */
function removeAnsiColors(text: string): string {
  return text.replace(
    // standard colors: 30-37, 90-97
    // background colors: 40-47, 100-107
    // truecolor / 256 color sequences are also removed
    /\x1B\[[0-9;]*m/g,
    "",
  );
}
...
function ansiToHtml(text: string): string {
  return DOMPurify.sanitize(
    colorConverter.ansi_to_html(removeAnsiColors(text)),
  );
}
